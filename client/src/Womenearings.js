  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/products") // Update URL if your server is hosted elsewhere
      .then(res => {
        ...
        // Filter directly for 'womenearings' - this is the correct category in AdminProducts.js
        const womenEarrings = res.data.filter(p => p.category === 'womenearings');
        
        // Normalize price fields for consistent UI display
        const normalizedProducts = womenEarrings.map(product => {
          return {
            ...product,
        ...
        console.log("Normalized women earrings products:", normalizedProducts);
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 className="mb-3">Women's Earrings</h2>
        <img 
          ...
          onError={(e) => {
            ...
            e.target.src = "https://via.placeholder.com/726x235?text=Women's+Earrings+Banner";
          }}
        />
      </div>
      ...
      {!loading && filteredProducts.length === 0 && (
        <div className="text-center my-5">
          <Alert variant="info">
            No earrings found. Please check back later or try different filters.
          </Alert>
        </div>
      )}
