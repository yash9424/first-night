  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/products") // Update URL if your server is hosted elsewhere
      .then(res => {
        ...
        // Filter for 'wedding' category - exact category name from AdminProducts.js
        const weddingProducts = res.data.filter(p => p.category === 'wedding');
        
        // Normalize price fields for consistent UI display
        const normalizedProducts = weddingProducts.map(product => {
          return {
            ...product,
        ...
        console.log("Filtered wedding collection products:", normalizedProducts);
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 className="mb-3">Wedding Collection</h2>
        <img 
          ...
          onError={(e) => {
            ...
            e.target.src = "https://via.placeholder.com/726x235?text=Wedding+Collection+Banner";
          }}
        />
      </div>
      ...
      {!loading && filteredProducts.length === 0 && (
        <div className="text-center my-5">
          <Alert variant="info">
            No wedding collection products found. Please check back later or try different filters.
          </Alert>
        </div>
      )}
