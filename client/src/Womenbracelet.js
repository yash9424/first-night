...
const Womenbracelet = () => {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/products") // Update URL if your server is hosted elsewhere
      .then(res => {
        const womenBracelets = res.data.filter(p => p.category === 'womenbracelet');
        console.log("All products:", res.data);
        console.log("Filtered women bracelets:", womenBracelets);
        setProducts(womenBracelets);
        setFilteredProducts(womenBracelets);
      })
      .catch(err => {
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 className="mb-3">Women's Bracelets</h2>
        <img 
