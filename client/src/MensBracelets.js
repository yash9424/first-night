...
const MensBracelets = () => {
  ...
  const getProducts = async () => {
    const res = await axios.get("http://localhost:5000/api/MensBracelets");
  ...
  const handleSubmit = async () => {
    try {
      if (editId) {
        await axios.put(`http://localhost:5000/api/MensBracelets/${editId}`, form);
      } else {
        await axios.post("http://localhost:5000/api/MensBracelets", form);
      }
      setShow(false);
      setForm({});
      setEditId(null);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this product?")) {
      await axios.delete(`http://localhost:5000/api/MensBracelets/${id}`);
      getProducts();
    }
  };
