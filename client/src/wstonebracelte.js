  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/products") // Update URL if your server is hosted elsewhere
      .then(res => {
        ...
        // Filter for 'wstonebracelte' category - exact category name from AdminProducts.js
        const stoneBraceletProducts = res.data.filter(p => p.category === 'wstonebracelte');
        
        // Normalize price fields for consistent UI display
        const normalizedProducts = stoneBraceletProducts.map(product => {
          return {
            ...product,
        ...
        console.log("Filtered stone bracelet products:", normalizedProducts);
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h2 className="mb-3">Women's Stone Bracelet</h2>
        <img 
          src="https://brantashop.com/cdn/shop/collections/Men.png?v=1722530043" 
          alt="Stone Bracelet Banner" 
          className="img-fluid rounded" 
          style={{ width: '726px', maxHeight: '235px', objectFit: 'cover' }} 
          onError={(e) => {
            e.target.onerror = null;
            e.target.src = "https://via.placeholder.com/726x235?text=Women's+Stone+Bracelet+Banner";
          }}
        />
      </div>
      ...
      {!loading && filteredProducts.length === 0 && (
        <div className="text-center my-5">
          <Alert variant="info">
            No stone bracelet products found. Please check back later or try different filters.
          </Alert>
        </div>
      )}
